...
namespace SevenZip
{
	...
	public:
        ...
        virtual HRESULT CompressDirectory(const TString& directory, ProgressCallback* callback, bool includeSubdirs = true);
		...
		virtual HRESULT CompressFiles( const TString& directory, const TString& searchFilter, ProgressCallback* callback, bool includeSubdirs = true );
        virtual HRESULT CompressAllFiles(const TString& directory, ProgressCallback* callback, bool includeSubdirs = true);

		// Compress just this single file as the root item in the archive.
        virtual HRESULT CompressFile(const TString& filePath, ProgressCallback* callback);
	...
	private:
        ...
        HRESULT CompressFilesToArchive(const TString& pathPrefix, const std::vector< intl::FilePathInfo >& filePaths, ProgressCallback* callback);
