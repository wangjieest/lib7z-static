...
//             RegisterArc(&NArchive::NNsis::g_ArcInfo);
            ...
            RegisterCodec(&NSBCJ::g_CodecInfo);
            RegisterCodec(&NSCopy::g_CodecInfo);
            RegisterCodec(&NSLZMA2::g_CodecInfo);
            RegisterCodec(&NSLZMA::g_CodecInfo);
//             RegisterCodec(&NSPPMD::g_CodecInfo);
//             RegisterCodec(&NSBZIP2::g_CodecInfo);
            RegisterCodec(&NSDeflate64::g_CodecInfo);
            RegisterCodec(&NSDeflate::g_CodecInfo);
            RegisterCodec(&NSDelta::g_CodecInfo);

            RegisterCodec(&NSByteSwap::g_CodecsInfo[0]);
            RegisterCodec(&NSByteSwap::g_CodecsInfo[1]);

//             RegisterCodec(&NSBranchMisc::g_CodecsInfo[0]);
//             RegisterCodec(&NSBranchMisc::g_CodecsInfo[1]);
//             RegisterCodec(&NSBranchMisc::g_CodecsInfo[2]);
//             RegisterCodec(&NSBranchMisc::g_CodecsInfo[3]);
//             RegisterCodec(&NSBranchMisc::g_CodecsInfo[4]);

//             RegisterCodec(&NSRAR5::g_CodecsInfo[0]);
//             RegisterCodec(&NSRAR5::g_CodecsInfo[1]);
//             RegisterCodec(&NSRAR5::g_CodecsInfo[2]);
//             RegisterCodec(&NSRAR5::g_CodecsInfo[3]);

            RegisterCodec(&NS7zAES::g_CodecInfo);
            RegisterCodec(&NSAES256CBC::g_CodecInfo);
