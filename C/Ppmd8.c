...
static CTX_PTR ReduceOrder(CPpmd8 *p, CPpmd_State *s1, CTX_PTR c)
{
  ...
  {
    CTX_PTR successor;
    CPpmd_State *sub_s1 = p->FoundState;
    p->FoundState = s;
...
static void UpdateModel(CPpmd8 *p)
{
  ...
  if (p->FoundState->Freq < MAX_FREQ / 4 && p->MinContext->Suffix != 0)
  {
    ...
    {
      so = STATS(c);
      if (so->Symbol != p->FoundState->Symbol)
      {
        do { so++; } while (so->Symbol != p->FoundState->Symbol);
        if (so[0].Freq >= so[-1].Freq)
        {
          SwapStates(&so[0], &so[-1]);
          so--;
        }
      }
      if (so->Freq < MAX_FREQ - 9)
      {
        so->Freq += 2;
        c->SummFreq += 2;
      }
  ...
  {
    CTX_PTR cs = CreateSuccessors(p, True, so, p->MinContext);
    if (cs == 0)
    {
  ...
  *p->Text++ = p->FoundState->Symbol;
  ...
  if (!fSuccessor)
  {
    CTX_PTR cs = ReduceOrder(p, so, p->MinContext);
    if (cs == NULL)
    {
      RESTORE_MODEL(c, 0);
      return;
  ...
  {
    CTX_PTR cs = CreateSuccessors(p, False, so, p->MinContext);
    if (cs == NULL)
    {
      RESTORE_MODEL(c, 0);
      return;
  ...
  for (; c != p->MinContext; c = SUFFIX(c))
  {
    ...
    {
      CPpmd_State *ssub = STATS(c) + ns1 + 1;
      SetSuccessor(ssub, successor);
      ssub->Symbol = fSymbol;
      ssub->Freq = (Byte)cf;
      c->Flags |= flag;
