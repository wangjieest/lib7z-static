...
NO_INLINE UInt32 CCoder::GetOptimal(UInt32 &backRes)
{
  ...
  for (;;)
  {
    ...
    UInt32 sub_numDistancePairs = m_MatchDistances[0];
    UInt32 newLen = 0;
    if (sub_numDistancePairs != 0)
    {
      newLen = matchDistances[sub_numDistancePairs - 2];
      if (newLen > m_NumFastBytes)
      {
        UInt32 len = Backward(backRes, cur);
        m_Optimum[cur].BackPrev = matchDistances[sub_numDistancePairs - 1];
    ...
    for (UInt32 lenTest = kMatchMinLen; ; lenTest++)
    {
      ...
      COptimal &sub_optimum = m_Optimum[cur + lenTest];
      if (curAndLenPrice < sub_optimum.Price)
      {
        sub_optimum.Price = curAndLenPrice;
        sub_optimum.PosPrev = (UInt16)cur;
        sub_optimum.BackPrev = (UInt16)distance;
      }
      if (lenTest == matchDistances[offs])
      {
        offs += 2;
        if (offs == sub_numDistancePairs)
          break;
        curPrice -= m_PosPrices[GetPosSlot(distance)];
