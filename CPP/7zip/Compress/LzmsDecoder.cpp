  ...
  {
    {
      ...
      { for (size_t idx = 0; idx < k_NumMainProbs; idx++) mainProbs[idx].Init(); }
      { for (size_t idx = 0; idx < k_NumMatchProbs; idx++) matchProbs[idx].Init(); }

      {
        for (size_t k = 0; k < k_NumReps; k++)
        {
          lzRepStates[k] = 0;
          for (size_t idx = 0; idx < k_NumRepProbs; idx++)
            lzRepProbs[k][idx].Init();
        }
      }
      {
        for (size_t k = 0; k < k_NumReps; k++)
