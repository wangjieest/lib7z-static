...
HRESULT CDecoder::Code(ISequentialInStream * const *in_streams, const UInt64 * const *inSizes, UInt32 numInStreams,
    ISequentialOutStream * const *outStreams, const UInt64 * const *outSizes, UInt32 numOutStreams,
    ICompressProgressInfo *progress)
{
    ...
    if (dec.state < BCJ2_NUM_STREAMS)
    {
      ...
      do
      {
        ...
        */
        ...
        HRESULT res2 = in_streams[dec.state]->Read(_bufs[dec.state] + totalRead, curSize, &curSize);
