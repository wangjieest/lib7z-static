...
#define NVmdkLE_16(offs, dest) dest = GetUi16(p + (offs));
#define NVmdkLE_32(offs, dest) dest = GetUi32(p + (offs));
#define NVmdkLE_64(offs, dest) dest = GetUi64(p + (offs));
...
bool CHeader::Parse(const Byte *p)
{
  ...
  NVmdkLE_32 (0x04, version);
  NVmdkLE_32 (0x08, flags);
  NVmdkLE_64 (0x0C, capacity);
  NVmdkLE_64 (0x14, grainSize);
  NVmdkLE_64 (0x1C, descriptorOffset);
  NVmdkLE_64 (0x24, descriptorSize);
  NVmdkLE_32 (0x2C, numGTEsPerGT);
  // NVmdkLE_64 (0x30, rgdOffset);
  NVmdkLE_64 (0x38, gdOffset);
  NVmdkLE_64 (0x40, overHead);
  // uncleanShutdown = buf[0x48];
  NVmdkLE_16(0x4D, algo);

  if (Is_NL() && GetUi32(p + 0x49) != 0x0A0D200A) // do we need Is_NL() check here?
    return false;
...
struct CMarker
{
  ...
  {
    NVmdkLE_64 (0, NumSectors);
    NVmdkLE_32 (8, SpecSize);
    NVmdkLE_32 (12, Type);
  }
};
      ...
      if (table.Size() != 0)
      {
        ...
        if (v != 0 && v != extent.ZeroSector)
        {
          ...
          if (extent.NeedDeflate)
          {
            ...
            if (GetUi64(_cacheCompressed) != (cluster << (clusterBits - 9)))
              return S_FALSE;

            UInt32 dataSize = GetUi32(_cacheCompressed + 8);
    ...
    unsigned numVols, unsigned volIndex, UInt64 &complexity)
{
  if (h.descriptorSize != 0)
  {
    ...
    if (h.descriptorOffset == 1 && h.Is_Marker() && GetUi64(DescriptorBuf) == 0)
    {
  ...
  {
    UInt32 v = GetUi32((const Byte *)table + (size_t)i * 4);
    ...
    {
      UInt32 v = GetUi32((const Byte *)buf + (size_t)k * 4);
      if (v == 0 || v == ZeroSector)
        continue;
      if (v < h.overHead)
        return S_FALSE;
  ...
  else if (lastSector != 0)
  {
    ...
    {
      UInt64 lba = GetUi64(buf);
      if (lba == (lastVirtCluster << (ClusterBits - 9)))
      {
        UInt32 dataSize = GetUi32(buf + 8);
        size_t dataSize2 = (size_t)dataSize + 12;
        dataSize2 = (dataSize2 + 511) & ~(size_t)511;
