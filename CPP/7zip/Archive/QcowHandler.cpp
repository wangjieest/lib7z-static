...
HRESULT CHandler::Open2(IInStream *stream, IArchiveOpenCallback *openCallback)
{
  ...
  _version = GetBe32(buf + 4);
  if (_version < 1 || _version > 3)
    return S_FALSE;
  
  const UInt64 backOffset = GetBe64(buf + 8);
  // UInt32 backSize = GetBe32(buf + 0x10);
  ...
  if (_version == 1)
  {
    // _mTime = GetBe32(buf + 0x14); // is unused im most images
    _size = GetBe64(buf + 0x18);
    ...
    if (_numMidBits < 1 || _numMidBits > 28)
      return S_FALSE;
    _cryptMethod = GetBe32(buf + 0x24);
    l1Offset = GetBe64(buf + 0x28);
    if (l1Offset < 0x30)
      return S_FALSE;
  ...
  {
    _clusterBits = GetBe32(buf + 0x14);
    if (_clusterBits < 9 || _clusterBits > 30)
      return S_FALSE;
    _numMidBits = _clusterBits - 3;
    _size = GetBe64(buf + 0x18);
    _cryptMethod = GetBe32(buf + 0x20);
    l1Size = GetBe32(buf + 0x24);
    l1Offset = GetBe64(buf + 0x28); // must be aligned for cluster
    
    UInt64 refOffset = GetBe64(buf + 0x30); // must be aligned for cluster
    UInt32 refClusters = GetBe32(buf + 0x38);
    
    // UInt32 numSnapshots = GetBe32(buf + 0x3C);
    // UInt64 snapshotsOffset = GetBe64(buf + 0x40); // must be aligned for cluster
    /*
    if (numSnapshots != 0)
      return S_FALSE;
    */
  ...
  for (UInt32 i = 0; i < l1Size; i++)
  {
    ...
    UInt64 v = GetBe64((const Byte *)table + (size_t)i * 8);
    v &= offsetMask;
    ...
    {
      UInt64 v = GetBe64((const Byte *)buf + (size_t)k);
      if (v == 0)
        continue;
