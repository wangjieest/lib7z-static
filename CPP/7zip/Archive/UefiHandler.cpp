...
#define NUefiGet24(p) (GetUi32(p) & 0xFFFFFF)
...
{
  return (GetUi32(p + 0x28) == kFvSignature && AreGuidsEq(p + kFfsGuidOffset, k_FFS_Guid));
}
...
public:
  ...
  bool Check(const Byte *p, UInt32 size)
  {
    ...
    if (IsThereTail())
      if (GetTailReference() != (UInt16)~GetUi16(p + Size - 2))
        return false;
...
#define G32(_offs_, dest) dest = GetUi32(p + (_offs_));
...
{
  PrintLevel(level);
  NUefiPRF(printf("%s, pos = %6x, size = %6d", name, posBase, size));
}
#else
...
HRESULT CHandler::ParseSections(int bufIndex, UInt32 posBase, UInt32 size, int parent, int method, unsigned level)
{
  ...
  for (;;)
  {
    if (size == pos)
      return S_OK;
    PrintLevel(level);
    NUefiPRF(printf("%s, pos = %6x", "Sect", pos));
    pos = (pos + 3) & ~(UInt32)3;
    if (pos > size)
      return S_FALSE;
    UInt32 rem = size - pos;
    ...
    UInt32 sectSize = NUefiGet24(p);
    if (sectSize > rem || sectSize < 4)
      return S_FALSE;
    
    Byte type = p[3];
    PrintLevel(level);
    NUefiPRF(printf("%s, type = %2x, pos = %6x, size = %6d", "Sect", type, pos, sectSize));
    CItem item;
    item.Method = method;
      ...
      else
      {
        ...
        else if (compressionType == COMPRESSION_TYPE_LZH)
        {
...

          {
            ...
            if (newSectSize < 8)
              return S_FALSE;
            UInt32 packSize = GetUi32(src);
            UInt32 unpackSize = GetUi32(src + 4);
        ...
        else
        {
          ...
          else
          {
            ...
            // UInt32 uncompressedSize2 = NUefiGet24(pStart);
            // Byte firstSectType = p[9 + 3];
            // firstSectType can be 0 in some archives
          }
          pStart += addSize;
          UInt64 lzmaUncompressedSize = GetUi64(pStart + 5);
    ...
    else if (type == SECTION_GUID_DEFINED)
    {
      ...
      if (sectSize < kHeaderSize)
        return S_FALSE;
      item.SetGuid(p + 4);
      UInt32 dataOffset = GetUi16(p + 4 + kGuidSize);
      UInt32 attrib = GetUi16(p + 4 + kGuidSize + 2);
      ...
      if (AreGuidsEq(p + 0x4, kGuids[kGuidIndex_CRC]) && propsSize == 4)
      {
        needDir = false;
        item.KeepName = false;
        if (CrcCalc(bufData + newOffset, newSectSize) != GetUi32(p + kHeaderSize))
          return S_FALSE;
      }
      else
      {
        ...
        case SECTION_RAW:
        {
          ...
          if (sectDataSize >= kFvHeaderSize + kInsydeOffset)
          {
            if (IsFfs(p + 4 + kInsydeOffset) &&
                sectDataSize - kInsydeOffset == GetUi64(p + 4 + kInsydeOffset + 0x20))
            {
              needAdd = false;
              item.Name = "vol";
              int newParent = AddDirItem(item);
...
{
  if (GetUi32(p + 0x28) != kFvSignature)
    return false;

  UInt32 attribs = GetUi32(p + 0x2C);
  if ((attribs & FVB_ERASE_POLARITY) == 0)
    return false;
  VolSize = GetUi64(p + 0x20);
  HeaderLen = GetUi16(p + 0x30);
    ...
    int parent, int method, int level)
{
  ...
  for (;;)
  {
    if (pos >= ffsHeader.HeaderLen)
      return S_FALSE;
    UInt32 numBlocks = GetUi32(p + pos);
    UInt32 length = GetUi32(p + pos + 4);
    pos += 8;
  ...
  for (;;)
  {
    ...
    PrintLevel(level); NUefiPRF(printf("%s, pos = %6x, size = %6d", "FILE", posBase + pos, fh.Size));
    ...
    UInt32 guid32 = GetUi32(fh.GuidName);
    bool full = true;
    ...
    PrintLevel(level);
    NUefiPRF(printf("%s", item.Characts));
