...
struct CChecksum
{
  ...
  UInt32 GetCrc32() const { return GetBe32(Data); }
...
HRESULT CFile::Parse(const Byte *p, UInt32 size)
{
  ...
  if (size < kHeadSize)
    return S_FALSE;
  if (GetBe32(p) != 0x6D697368) // "mish" signature
    return S_FALSE;
  if (GetBe32(p + 4) != 1) // version
    return S_FALSE;
  // UInt64 firstSectorNumber = GetBe64(p + 8);
  UInt64 numSectors = GetBe64(p + 0x10);

  StartPos = GetBe64(p + 0x18);

  // UInt32 decompressedBufRequested = GetBe32(p + 0x20); // ???
  // UInt32 blocksDescriptor = GetBe32(p + 0x24); // number starting from -1?
  // char Reserved1[24];
  
  Checksum.Parse(p + 0x40);
  PRF(printf("\n\nChecksum Type = %2d", Checksum.Type));

  UInt32 numBlocks = GetBe32(p + 0xC8);
  ...
  for (i = 0; i < numBlocks; i++, p += kRecordSize)
  {
    CBlock b;
    b.Type = GetBe32(p);
    b.UnpPos   = GetBe64(p + 0x08) << 9;
    b.UnpSize  = GetBe64(p + 0x10) << 9;
    b.PackPos  = GetBe64(p + 0x18);
    b.PackSize = GetBe64(p + 0x20);
    ...
    PRF(printf("\nType=%8x  m[1]=%8x  uPos=%8x  uSize=%7x  pPos=%8x  pSize=%7x",
        b.Type, GetBe32(p + 4), (UInt32)b.UnpPos, (UInt32)b.UnpSize, (UInt32)b.PackPos, (UInt32)b.PackSize));
...
static inline bool IsKoly(const Byte *p)
{
  ...
  /*
  if (GetBe32(p) != 0x6B6F6C79) // "koly" signature
    return false;
  if (GetBe32(p + 4) != 4) // version
    return false;
  if (GetBe32(p + 8) != HEADER_SIZE)
    return false;
  return true;
  */
}
...
HRESULT CHandler::Open2(IInStream *stream)
{
  ...
  // UInt32 flags = GetBe32(buf + 12);
  // UInt64 runningDataForkOffset = GetBe64(buf + 0x10);
  UInt64 dataForkOffset = GetBe64(buf + 0x18);
  UInt64 dataForkLen = GetBe64(buf + 0x20);
  UInt64 rsrcOffset = GetBe64(buf + 0x28);
  UInt64 rsrcLen = GetBe64(buf + 0x30);
  // UInt32 segmentNumber = GetBe32(buf + 0x38);
  // UInt32 segmentCount = GetBe32(buf + 0x3C);
  // Byte segmentGUID[16];
  // CChecksum dataForkChecksum;
  // dataForkChecksum.Parse(buf + 0x50);
  UInt64 xmlOffset = GetBe64(buf + 0xD8);
  UInt64 xmlLen = GetBe64(buf + 0xE0);
  ...
  // UInt32 imageVariant = GetBe32(buf + 0x1E8);
  // UInt64 numSectors = GetBe64(buf + 0x1EC);
  // Byte reserved[0x12]
  ...
  if (useRsrc)
  {
    ...
    UInt32 headSize = GetBe32(p + 0);
    UInt32 footerOffset = GetBe32(p + 4);
    UInt32 mainDataSize = GetBe32(p + 8);
    UInt32 footerSize = GetBe32(p + 12);
    ...
    if ((UInt32)GetBe16(p + 0x18) != 0x1C)
      return S_FALSE;
    UInt32 namesOffset = GetBe16(p + 0x1A);
    if (namesOffset > footerSize)
      return S_FALSE;
    
    UInt32 numItems = (UInt32)GetBe16(p + 0x1C) + 1;
    ...
    for (UInt32 i = 0; i < numItems; i++)
    {
      ...
      UInt32 typeId = GetBe32(p2);
      if (typeId != 0x626C6B78) // blkx
        continue;
      
      UInt32 numFiles = (UInt32)GetBe16(p2 + 4) + 1;
      UInt32 offs = GetBe16(p2 + 6);
