...
#define NElfG16(offs, v) v = Get16(p + (offs), be)
#define NElfG32(offs, v) v = Get32(p + (offs), be)
#define NElfG64(offs, v) v = Get64(p + (offs), be)
...
bool CHeader::Parse(const Byte *p)
{
  ...
  NElfG16(0x10, Type);
  NElfG16(0x12, Machine);
  if (Get32(p + 0x14, be) != 1) // Version
    return false;

  if (Mode64)
  {
    // NElfG64(0x18, EntryVa);
    NElfG64(0x20, ProgOffset);
    NElfG64(0x28, SectOffset);
    p += 0x30;
  }
  else
  {
    // NElfG32(0x18, EntryVa);
    NElfG32(0x1C, ProgOffset);
    NElfG32(0x20, SectOffset);
    p += 0x24;
  }

  NElfG32(0, Flags);
  NElfG16(4, HeaderSize);
  if (HeaderSize != (Mode64 ? kHeaderSize64 : kHeaderSize32))
    return false;

  NElfG16(6, SegmentEntrySize);
  NElfG16(8, NumSegments);
  NElfG16(10, SectionEntrySize);
  NElfG16(12, NumSections);
  NElfG16(14, NamesSectIndex);
...
bool CSection::Parse(const Byte *p, bool mode64, bool be)
{
  ...
  if (mode64)
  {
    ...
    NElfG32(0x28, Link);
    NElfG32(0x2C, Info);
    NElfG64(0x30, AddrAlign);
    NElfG64(0x38, EntSize);
  }
  else
  {
    NElfG32(0x08, Flags);
    ...
    NElfG32(0x18, Link);
    NElfG32(0x1C, Info);
    NElfG32(0x20, AddrAlign);
    NElfG32(0x24, EntSize);
  }
