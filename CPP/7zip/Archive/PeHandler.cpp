...
#define NPeG16(offs, v) v = GetUi16(p + (offs))
#define NPeG32(offs, v) v = GetUi32(p + (offs))
#define NPeG64(offs, v) v = GetUi64(p + (offs))
...
{
  if (GetUi32(p) != k_Signature32)
    return false;
  p += 4;
  NPeG16( 0, Machine);
  NPeG16( 2, NumSections);
  NPeG32( 4, Time);
  NPeG32( 8, PointerToSymbolTable);
  NPeG32(12, NumSymbols);
  NPeG16(16, OptHeaderSize);
  NPeG16(18, Flags);
...
bool COptHeader::Parse(const Byte *p, UInt32 size)
{
  ...
  NPeG32( 4, CodeSize);
  NPeG32( 8, InitDataSize);
  NPeG32(12, UninitDataSize);
  // NPeG32(16, AddressOfEntryPoint);
  // NPeG32(20, BaseOfCode);
  ...
  // reserved = GetUi32(p + 52);

  NPeG32(56, ImageSize);
  NPeG32(60, HeadersSize);
  NPeG32(64, CheckSum);
  NPeG16(68, SubSystem);
  NPeG16(70, DllCharacts);
  ...
  if (Is64Bit())
  {
    ...
    // BaseOfData32 = 0;
    NPeG64(24, ImageBase);
    NPeG64(72, StackReserve);
    NPeG64(80, StackCommit);
    NPeG64(88, HeapReserve);
    NPeG64(96, HeapCommit);
    pos = 108;
  }
  else
  {
    // NPeG32(24, BaseOfData32);
    NPeG32(28, ImageBase);
    NPeG32(72, StackReserve);
    NPeG32(76, StackCommit);
    NPeG32(80, HeapReserve);
    NPeG32(84, HeapCommit);
    pos = 92;
  }

  NPeG32(pos, NumDirItems);
  if (NumDirItems > (1 << 16))
    return false;
  pos += 4;
...
HRESULT CHandler::ReadString(UInt32 offset, UString &dest) const
{
  ...
  if (rem < 2)
    return S_FALSE;
  unsigned len = GetUi16(_buf + offset);
  if ((rem - 2) / 2 < len)
    return S_FALSE;
  dest.Empty();
  ...
  for (i = 0; i < len; i++)
  {
    wchar_t c = (wchar_t)GetUi16(src + i * 2);
    if (c == 0)
      break;
    destBuf[i] = c;
  }
...
HRESULT CHandler::ReadTable(UInt32 offset, CRecordVector<CTableItem> &items)
{
  ...
  if (rem < 16)
    return S_FALSE;
  unsigned numNameItems = GetUi16(_buf + offset + 12);
  unsigned numIdItems = GetUi16(_buf + offset + 14);
  ...
  for (unsigned i = 0; i < numItems; i++, offset += 8)
  {
    ...
    CTableItem item;
    item.ID = GetUi32(buf + 0);
    if ((bool)((item.ID & kFlag) != 0) != (bool)(i < numNameItems))
      return S_FALSE;
    item.Offset = GetUi32(buf + 4);
...
{
  if (size < kBitmapInfoHeader_Size || GetUi32(p) != kBitmapInfoHeader_Size)
    return false;
  NPeG32( 4, XSize);
  NPeG32( 8, YSize);
  NPeG16(12, Planes);
  NPeG16(14, BitCount);
  NPeG32(16, Compression);
  NPeG32(20, SizeImage);
  return true;
}
...
bool CHandler::ParseStringRes(UInt32 id, UInt32 lang, const Byte *src, UInt32 size)
{
  ...
  for (i = 0; i < 16; i++)
  {
    ...
    if (len != 0)
    {
      ...
      for (j = 0; j < len; j++, pos += 2)
        item.AddWChar_Smart(GetUi16(src + pos));
      item.NewLine();
    }
  }
  if (size == pos)
  ...
  if (size == pos + 2 && GetUi16(src + pos) == 0)
    return true;
...
{
  if (GetUi32(p) != kMy_VS_FFI_SIGNATURE) // signature;
    return false;
  // NPeG32(0x04, StrucVersion);
  NPeG32(0x08, VersionMS);
  NPeG32(0x0C, VersionLS);
  NPeG32(0x10, ProductVersionMS);
  NPeG32(0x14, ProductVersionLS);
  NPeG32(0x18, FlagsMask);
  NPeG32(0x1C, Flags);
  NPeG32(0x20, OS);
  NPeG32(0x24, Type);
  NPeG32(0x28, Subtype);
  ...
  FOR_VECTOR (i, specItems)
  {
    ...
    FOR_VECTOR (j, specItems2)
    {
      ...
      FOR_VECTOR (k, specItems3)
      {
        ...
        item.Lang = item3.ID;
        item.Offset = GetUi32(buf + 0);
        item.Size = GetUi32(buf + 4);
        // UInt32 codePage = GetUi32(buf + 8);
        if (GetUi32(buf + 12) != 0)
          return S_FALSE;
...
{
  {
    ...
    /* most of PE files contain 0x0090 at offset 2.
       ...
       if (GetUi16(h + 2) != 0x90) return false; */
    _peOffset = GetUi32(h + 0x3C);
...
bool CHeader::Parse(const Byte *p)
{
  ...
  SubSystem = p[5];
  NPeG16(2, Machine);
  NPeG16(6, StrippedSize);
  /*
  NPeG32(8, AddressOfEntryPoint);
  NPeG32(12, BaseOfCode);
  NPeG64(16, ImageBase);
  */
  for (int i = 0; i < 2; i++)
  {
