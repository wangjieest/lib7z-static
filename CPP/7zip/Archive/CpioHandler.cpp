...
static UInt16 NCpioGet16(const Byte *p, bool be) { if (be) return GetBe16(p); return GetUi16(p); }
static UInt32 NCpioGet32(const Byte *p, bool be) { return ((UInt32)NCpioGet16(p, be) << 16) + NCpioGet16(p + 2, be); }

#define NCpioG16(offs, v) v = NCpioGet16(p + (offs), be)
#define NCpioG32(offs, v) v = NCpioGet32(p + (offs), be)
...
#define NCpioREAD_STREAM(_dest_, _size_) \
...
HRESULT CInArchive::GetNextItem(CItem &item, EErrorType &errorType)
{
  ...
  NCpioREAD_STREAM(p, k_BinRecord_Size)
  ...
  if (p[0] != '0')
  {
    ...
    NCpioG16(2, item.DevMinor);
    NCpioG16(4, item.inode);
    NCpioG16(6, item.Mode);
    NCpioG16(8, item.UID);
    NCpioG16(10, item.GID);
    NCpioG16(12, item.NumLinks);
    NCpioG16(14, item.RDevMinor);
    NCpioG32(16, item.MTime);
    NCpioG16(20, nameSize);
    NCpioG32(22, item.Size);
  ...
  else
  {
    ...
    if (p[5] == kMagicOct)
    {
      ...
      NCpioREAD_STREAM(p + k_BinRecord_Size, k_OctRecord_Size - k_BinRecord_Size)
      item.Align = 1;
      item.DevMajor = 0;
      item.RDevMajor = 0;
    ...
    else
    {
      ...
      NCpioREAD_STREAM(p + k_BinRecord_Size, k_HexRecord_Size - k_BinRecord_Size)
