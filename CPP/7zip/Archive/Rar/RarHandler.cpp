...
static const Byte kMarker[NHeader::kMarkerSize] = { 0x52 , 0x61, 0x72, 0x21, 0x1a, 0x07, 0x00 };
...
{
  return GetUi16(header) == (UInt16)(CrcCalc(header + 2, headerSize - 2) & 0xFFFF);
}
...
HRESULT CInArchive::Open(IInStream *stream, const UInt64 *searchHeaderSizeLimit)
{
  ...
  UInt32 blockSize = GetUi16(buf + 5);

  ArcInfo.EncryptVersion = 0;
  ArcInfo.Flags = GetUi16(buf + 3);
...
#define READ_TIME_2(_mask_, _def_, _ttt_) \
    ...
    { if (size < 4) return false; \
      _ttt_ .DosTime = GetUi32(p); p += 4; size -= 4; \
...
bool CInArchive::ReadHeaderReal(const Byte *p, unsigned size, CItem &item)
{
  ...
  item.PackSize = GetUi32(p);
  item.Size = GetUi32(p + 4);
  item.HostOS = p[8];
  item.FileCRC = GetUi32(p + 9);
  item.MTime.DosTime = GetUi32(p + 13);
  item.UnPackVersion = p[17];
  item.Method = p[18];
  unsigned nameSize = GetUi16(p + 19);
  item.Attrib = GetUi32(p + 21);
  ...
  if ((item.Flags & NHeader::NFile::kSize64Bits) != 0)
  {
    if (size < 8)
      return false;
    item.PackSize |= ((UInt64)GetUi32(p) << 32);
    item.Size |= ((UInt64)GetUi32(p + 4) << 32);
    p += 8;
    size -= 8;
  }
  if (nameSize > size)
...
HRESULT CInArchive::GetNextItem(CItem &item, ICryptoGetTextPassword *getTextPassword, bool &filled, EErrorType &error)
{
  ...
  for (;;)
  {
    ...
    m_BlockHeader.CRC = GetUi16(p + 0);
    m_BlockHeader.Type = p[2];
    m_BlockHeader.Flags = GetUi16(p + 3);
    m_BlockHeader.HeadSize = GetUi16(p + 5);
      ...
      else
      {
        ...
        if (m_BlockHeader.Flags & NHeader::NArchive::kEndOfArc_Flags_DataCRC)
        {
          if (processed < offset + 4)
            ...
            ArcInfo.DataCRC = GetUi32(m_FileHeaderData + offset);
          offset += 4;
        }
        ...
        if (m_BlockHeader.Flags & NHeader::NArchive::kEndOfArc_Flags_VolNumber)
        {
          if (processed < offset + 2)
            error = k_ErrorType_Corrupted;
          ArcInfo.VolNumber = (UInt32)GetUi16(m_FileHeaderData + offset);
        }
    ...
    if ((m_BlockHeader.Flags & NHeader::NBlock::kLongBlock) != 0)
    {
      ...
      UInt32 dataSize = GetUi32(m_FileHeaderData + 7);
