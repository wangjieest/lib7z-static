  ...
  */
  ...
  if (item.IcbTag.IsDir())
  {
    if (!item.CheckChunkSizes() || !CheckItemExtents(volIndex, item))
      return S_FALSE;
    CByteBuffer buff;
    RINOK(ReadFromFile(volIndex, item, buff));
    item.Size = 0;
    item.Extents.ClearAndFree();
    item.InlineData.Free();

    const Byte *pbuff = buff;
    size = buff.Size();
    ...
    for (; processedTotal < size;)
    {
      size_t processedCur;
      CFileId fileId;
      RINOK(fileId.Parse(pbuff + processedTotal, size - processedTotal, processedCur));
    ...
    else if (tag.Id == DESC_TYPE_LogicalVol)
    {
      ...
      for (UInt32 i = 0; i < numPartitionMaps; i++)
      {
        ...
        if (pm.Type == 1)
        {
          ...
          pm.PartitionNumber = Get16(buf + npos + 4);
        ...
        else if (pm.Type == 2)
        {
          ...
          if (Get16(buf + npos + 2) != 0)
            return S_FALSE;
          // pm.VolSeqNumber = Get16(buf + pos + 36);
          pm.PartitionNumber = Get16(buf + npos + 38);
  ...
  for (volIndex = 0; volIndex < LogVols.Size(); volIndex++)
  {
    ...
    // for (int i = 0; i < 1; i++)
    {
      if (nextExtent.GetLen() < 512)
        return S_FALSE;
      CByteBuffer buff(nextExtent.GetLen());
      RINOK(Read(volIndex, nextExtent, buff));
      const Byte *p = buff;
