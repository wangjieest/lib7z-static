...
/*
#define NSquashfsGet16(p) (be ? GetBe16(p) : GetUi16(p))
#define NSquashfsGet32(p) (be ? GetBe32(p) : GetUi32(p))
#define NSquashfsGet64(p) (be ? GetBe64(p) : GetUi64(p))
*/
...
#define NSquashfsGet16(p) Get16b(p, be)
#define NSquashfsGet32(p) Get32b(p, be)
#define NSquashfsGet64(p) Get64b(p, be)

#define NSquashfsLE_16(offs, dest) dest = GetUi16(p + (offs));
#define NSquashfsLE_32(offs, dest) dest = GetUi32(p + (offs));
#define NSquashfsLE_64(offs, dest) dest = GetUi64(p + (offs));

#define NSquashfsGET_16(offs, dest) dest = NSquashfsGet16(p + (offs));
#define NSquashfsGET_32(offs, dest) dest = NSquashfsGet32(p + (offs));
#define NSquashfsGET_64(offs, dest) dest = NSquashfsGet64(p + (offs));
...
struct CHeader
{
  ...
  {
    Method = kMethod_ZLIB;
    NSquashfsGET_32 (0x08, Size);
    NSquashfsGET_32 (0x0C, UidTable);
    NSquashfsGET_32 (0x10, GidTable);
    NSquashfsGET_32 (0x14, InodeTable);
    NSquashfsGET_32 (0x18, DirTable);
    NSquashfsGET_16 (0x20, BlockSize);
    NSquashfsGET_16 (0x22, BlockSizeLog);
    Flags   = p[0x24];
    NumUids = p[0x25];
    NumGids = p[0x26];
    NSquashfsGET_32 (0x27, CTime);
    NSquashfsGET_64 (0x2B, RootInode);
    NumFrags = 0;
    FragTable = UidTable;

    if (Major >= 2)
    {
      NSquashfsGET_32 (0x33, BlockSize);
      NSquashfsGET_32 (0x37, NumFrags);
      NSquashfsGET_32 (0x3B, FragTable);
      if (Major == 3)
      {
        NSquashfsGET_64 (0x3F, Size);
        NSquashfsGET_64 (0x47, UidTable);
        NSquashfsGET_64 (0x4F, GidTable);
        NSquashfsGET_64 (0x57, InodeTable);
        NSquashfsGET_64 (0x5F, DirTable);
        NSquashfsGET_64 (0x67, FragTable);
        NSquashfsGET_64 (0x6F, LookupTable);
      }
    }
  }
  ...
  {
    NSquashfsLE_32 (0x08, CTime);
    NSquashfsLE_32 (0x0C, BlockSize);
    NSquashfsLE_32 (0x10, NumFrags);
    NSquashfsLE_16 (0x14, Method);
    NSquashfsLE_16 (0x16, BlockSizeLog);
    NSquashfsLE_16 (0x18, Flags);
    NSquashfsLE_16 (0x1A, NumIDs);
    NSquashfsLE_64 (0x20, RootInode);
    NSquashfsLE_64 (0x28, Size);
    NSquashfsLE_64 (0x30, UidTable);
    NSquashfsLE_64 (0x38, XattrIdTable);
    NSquashfsLE_64 (0x40, InodeTable);
    NSquashfsLE_64 (0x48, DirTable);
    NSquashfsLE_64 (0x50, FragTable);
    NSquashfsLE_64 (0x58, LookupTable);
    GidTable = 0;
  }
  ...
  bool Parse(const Byte *p)
  {
    ...
    NSquashfsGET_32 (4, NumInodes);
    NSquashfsGET_16 (0x1C, Major);
    NSquashfsGET_16 (0x1E, Minor);
    if (Major <= 3)
      Parse3(p);
    else
    {
...
UInt32 CNode::Parse1(const Byte *p, UInt32 size, const CHeader &_h)
{
  ...
  if (Type == kType_FILE)
  {
    if (size < 15)
      return 0;
    // NSquashfsGET_32 (3, MTime);
    NSquashfsGET_32 (7, StartBlock);
    UInt32 t;
    NSquashfsGET_32 (11, t);
    FileSize = t;
  ...
  if (Type == kType_DIR)
  {
    ...
    // NSquashfsGET_32 (7, MTime);
    NSquashfsGET_32 (10, StartBlock);
    if (be)
      StartBlock &= 0xFFFFFF;
    else
      StartBlock >>= 8;
  ...
  {
    UInt32 len;
    NSquashfsGET_16 (3, len);
    FileSize = len;
    len += 5;
    return (len <= size) ? len : 0;
  }

  // NSquashfsGET_32 (3, RDev);
  return 5;
}
...
UInt32 CNode::Parse2(const Byte *p, UInt32 size, const CHeader &_h)
{
  ...
  if (Type == kType_FILE)
  {
    if (size < 24)
      return 0;
    // NSquashfsGET_32 (4, MTime);
    NSquashfsGET_32 (8, StartBlock);
    NSquashfsGET_32 (12, Frag);
    NSquashfsGET_32 (16, Offset);
    UInt32 t;
    NSquashfsGET_32 (20, t);
    FileSize = t;
    UInt32 numBlocks = t >> _h.BlockSizeLog;
  ...
  if (Type == kType_DIR)
  {
    ...
    // NSquashfsGET_32 (8, MTime);
    NSquashfsGET_32 (11, StartBlock);
    if (be)
      StartBlock &= 0xFFFFFF;
    else
      StartBlock >>= 8;
  ...
  if (Type == kType_DIR + 7)
  {
    ...
    // NSquashfsGET_32 (9, MTime);
    NSquashfsGET_32 (12, StartBlock);
    if (be)
      StartBlock &= 0xFFFFFF;
    else
      StartBlock >>= 8;
    UInt32 iCount;
    NSquashfsGET_16 (16, iCount);
    UInt32 pos = 18;
...
UInt32 CNode::Parse3(const Byte *p, UInt32 size, const CHeader &_h)
{
  ...
  if (Type == kType_FILE || Type == kType_FILE + 7)
  {
    ...
    if (Type == kType_FILE)
    {
      if (size < 32)
        return 0;
      NSquashfsGET_64 (12, StartBlock);
      NSquashfsGET_32 (20, Frag);
      NSquashfsGET_32 (24, Offset);
      NSquashfsGET_32 (28, FileSize);
      offset = 32;
    }
    else
    {
      if (size < 40)
        return 0;
      // NSquashfsGET_32 (12, NumLinks);
      NSquashfsGET_64 (16, StartBlock);
      NSquashfsGET_32 (24, Frag);
      NSquashfsGET_32 (28, Offset);
      NSquashfsGET_64 (32, FileSize);
      offset = 40;
    }
  ...
  if (Type == kType_DIR)
  {
    ...
    NSquashfsGET_32 (20, StartBlock);
    // NSquashfsGET_32 (24, Parent);
    return 28;
  }
  ...
  if (Type == kType_DIR + 7)
  {
    ...
    NSquashfsGET_32 (21, StartBlock);
    UInt32 iCount;
    NSquashfsGET_16 (25, iCount);
    // NSquashfsGET_32 (27, Parent);
    UInt32 pos = 31;
    for (UInt32 i = 0; i < iCount; i++)
    {
      // UInt32 index
...
UInt32 CNode::Parse4(const Byte *p, UInt32 size, const CHeader &_h)
{
  if (size < 20)
    return 0;
  NSquashfsLE_16 (0, Type);
  NSquashfsLE_16 (2, Mode);
  NSquashfsLE_16 (4, Uid);
  NSquashfsLE_16 (6, Gid);
  // NSquashfsLE_32 (8, MTime);
  // NSquashfsLE_32 (12, Number);
  ...
  if (Type == kType_FILE || Type == kType_FILE + 7)
  {
    ...
    if (Type == kType_FILE)
    {
      if (size < 32)
        return 0;
      NSquashfsLE_32 (16, StartBlock);
      NSquashfsLE_32 (20, Frag);
      NSquashfsLE_32 (24, Offset);
      NSquashfsLE_32 (28, FileSize);
      offset = 32;
    }
    else
    {
      if (size < 56)
        return 0;
      NSquashfsLE_64 (16, StartBlock);
      NSquashfsLE_64 (24, FileSize);
      // NSquashfsLE_64 (32, Sparse);
      // NSquashfsLE_32 (40, NumLinks);
      NSquashfsLE_32 (44, Frag);
      NSquashfsLE_32 (48, Offset);
      // NSquashfsLE_32 (52, Xattr);
      offset = 56;
    }
    UInt64 pos = GetNumBlocks(_h) * 4 + offset;
    return (pos <= size) ? (UInt32)pos : 0;
  ...
  if (Type == kType_DIR)
  {
    if (size < 32)
      return 0;
    NSquashfsLE_32 (16, StartBlock);
    // NSquashfsLE_32 (20, NumLinks);
    NSquashfsLE_16 (24, FileSize);
    NSquashfsLE_16 (26, Offset);
    // NSquashfsLE_32 (28, Parent);
    return 32;
  }
  
  // NSquashfsLE_32 (16, NumLinks);

  if (Type == kType_DIR + 7)
  {
    if (size < 40)
      return 0;
    NSquashfsLE_32 (20, FileSize);
    NSquashfsLE_32 (24, StartBlock);
    // NSquashfsLE_32 (28, Parent);
    UInt32 iCount;
    NSquashfsLE_16 (32, iCount);
    NSquashfsLE_16 (34, Offset);
    // NSquashfsLE_32 (36, Xattr);
...
HRESULT CHandler::ReadMetadataBlock(UInt32 &packSize)
{
  ...
  bool be = _h.be;
  UInt32 size = NSquashfsGet16(temp);
  ...
  while (rem != 0)
  {
    ...
    if (_h.Major <= 2)
    {
      if (rem < 4)
        return S_FALSE;
      count = p[0];
      tempItem.StartBlock = NSquashfsGet32(p);
      if (be)
    ...
    else
    {
      ...
      NSquashfsGET_32 (0, tempItem.StartBlock);
      // NSquashfsGET_32 (4, tempItem.iNodeNumber1);
      p += 8;
      rem -= 8;
    }
    count++;
    ...
    for (UInt32 i = 0; i < count; i++)
    {
      ...
      if (_h.IsOldVersion())
      {
        ...
        /*
        if (_h.Major > 2)
          tempItem.iNodeNumber2 = NSquashfsGet16(p + 3);
        */
      }
      else
      {
        NSquashfsGET_16 (0, tempItem.Offset);
        // NSquashfsGET_16 (2, tempItem.iNodeNumber2);
        NSquashfsGET_16 (4, tempItem.Type);
        NSquashfsGET_16 (6, size);
      }
      p += nameOffset;
      rem -= nameOffset;
      size++;
...
HRESULT CHandler::Open2(IInStream *inStream)
{
  ...
  if (_h.NumFrags != 0)
  {
    ...
    {
      UInt64 offset = bigFrag ? NSquashfsGet64(data + i * 8) : NSquashfsGet32(data + i * 4);
      ...
      for (UInt32 j = 0; j < kMetadataBlockSize && j < unpackSize;)
      {
        ...
        if (bigFrag)
        {
          frag.StartBlock = NSquashfsGet64(buf + j);
          frag.Size = NSquashfsGet32(buf + j + 8);
          // some archives contain nonzero in unused (buf + j + 12)
          j += 16;
        }
        else
        {
          frag.StartBlock = NSquashfsGet32(buf + j);
          frag.Size = NSquashfsGet32(buf + j + 4);
          j += 8;
        }
        _frags.Add(frag);
      }
...
AString CHandler::GetPath(int index) const
{
  ...
  do
  {
    ...
    unsigned size = (_h.IsOldVersion() ? (unsigned)p[2] : (unsigned)NSquashfsGet16(p + 6)) + 1;
  ...
  switch (propID)
  {
    ...
    case kpidMTime:
    {
      ...
      if (offset != 0)
      {
        ...
        FILETIME ft;
        NWindows::NTime::UnixTimeToFileTime(NSquashfsGet32(p), ft);
        prop = ft;
      }
      break;
    }
    ...
    case kpidUser:
    {
      ...
      if (offset < _uids.Size())
        prop = (UInt32)NSquashfsGet32(_uids + offset);
      break;
    }
    case kpidGroup:
    {
      ...
      else
      {
        ...
        if (offset < _gids.Size())
          prop = (UInt32)NSquashfsGet32(_gids + offset);
      }
      break;
    }
    */
