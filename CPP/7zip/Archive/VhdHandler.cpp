...
#define NVhdG32(_offs_, dest) dest = GetBe32(p + (_offs_));
#define NVhdG64(_offs_, dest) dest = GetBe64(p + (_offs_));
...
static const Byte kSignature[kSignatureSize] = { 'c', 'o', 'n', 'e', 'c', 't', 'i', 'x', 0, 0 };
...
static bool CheckBlock(const Byte *p, unsigned size, unsigned checkSumOffset, unsigned zeroOffset)
{
  ...
  for (i = checkSumOffset + 4; i < size; i++)
    sum += p[i];
  if (~sum != GetBe32(p + checkSumOffset))
    return false;
...
bool CFooter::Parse(const Byte *p)
{
  if (memcmp(p, kSignature, kSignatureSize) != 0)
    return false;
  // NVhdG32(0x08, Features);
  // NVhdG32(0x0C, FormatVersion);
  NVhdG64(0x10, DataOffset);
  NVhdG32(0x18, CTime);
  NVhdG32(0x1C, CreatorApp);
  NVhdG32(0x20, CreatorVersion);
  NVhdG32(0x24, CreatorHostOS);
  // NVhdG64(0x28, OriginalSize);
  NVhdG64(0x30, CurrentSize);
  NVhdG32(0x38, DiskGeometry);
  NVhdG32(0x3C, Type);
...
struct CParentLocatorEntry
{
  ...
  {
    NVhdG32(0x00, Code);
    NVhdG32(0x04, DataSpace);
    NVhdG32(0x08, DataLen);
    NVhdG64(0x10, DataOffset);
    return GetBe32(p + 0x0C) == 0; // Reserved
  }
};
...
bool CDynHeader::Parse(const Byte *p)
{
  if (memcmp(p, "cxsparse", 8) != 0)
    return false;
  // NVhdG64(0x08, DataOffset);
  NVhdG64(0x10, TableOffset);
  // NVhdG32(0x18, HeaderVersion);
  NVhdG32(0x1C, NumBlocks);
  {
    UInt32 blockSize = GetBe32(p + 0x20);
    unsigned i;
  ...
  NVhdG32(0x38, ParentTime);
  if (GetBe32(p + 0x3C) != 0) // reserved
    return false;
