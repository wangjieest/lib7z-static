...
API_FUNC_static_IsArc IsArc_Arj(const Byte *p, size_t size)
{
  ...
  if (blockSize + 4 <= size)
    if (GetUi32(p + blockSize) != CrcCalc(p, blockSize))
      return k_IsArc_Res_NO;
...
HRESULT CArcHeader::Parse(const Byte *p, unsigned size)
{
  ...
  CTime = GetUi32(p + 8);
  MTime = GetUi32(p + 12);
  ArchiveSize = GetUi32(p + 16); // it can be zero. (currently used only for secured archives)
  // SecurPos = GetUi32(p + 20);
  // UInt16 filespecPositionInFilename = GetUi16(p + 24);
  SecurSize = GetUi16(p + 26);
...
HRESULT CItem::Parse(const Byte *p, unsigned size)
{
  ...
  // Reserved = p[7];
  MTime = GetUi32(p + 8);
  PackSize = GetUi32(p + 12);
  Size = GetUi32(p + 16);
  FileCRC = GetUi32(p + 20);
  // FilespecPosInFilename = GetUi16(p + 24);
  FileAccessMode = GetUi16(p + 26);
  // FirstChapter = p[28];
  // FirstChapter = p[29];

  SplitPos = 0;
  if (IsSplitBefore() && headerSize >= 34)
    SplitPos = GetUi32(p + 30);
...
HRESULT CArc::ReadBlock(bool &filled, bool readSignature)
{
  ...
  _blockSize = GetUi16(buf + signSize);
  ...
  if (GetUi32(_block + _blockSize) != CrcCalc(_block, _blockSize))
  {
    Error = k_ErrorType_Corrupted;
    return S_OK;
  }
