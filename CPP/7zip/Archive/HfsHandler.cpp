...
void CFork::Parse(const Byte *p)
{
  Extents.Clear();
  Size = GetBe64(p);
  // ClumpSize = GetBe32(p + 8);
  NumBlocks = GetBe32(p + 12);
  p += 16;
  for (unsigned i = 0; i < kNumFixedExtents; i++, p += 8)
  {
    CExtent e;
    e.Pos = GetBe32(p);
    e.NumBlocks = GetBe32(p + 4);
    if (e.NumBlocks != 0)
      Extents.Add(e);
  }
}
...
{
  // TreeDepth = GetBe16(p);
  // RootNode = GetBe32(p + 2);
  // LeafRecords = GetBe32(p + 6);
  FirstLeafNode = GetBe32(p + 0xA);
  // LastLeafNode = GetBe32(p + 0xE);
  UInt32 nodeSize = GetBe16(p + 0x12);
  ...
  // MaxKeyLength = GetBe16(p + 0x14);
  TotalNodes = GetBe32(p + 0x16);
  // FreeNodes = GetBe32(p + 0x1A);
  // Reserved1 = GetBe16(p + 0x1E);
  // ClumpSize = GetBe32(p + 0x20);
  // BtreeType = p[0x24];
  // KeyCompareType = p[0x25];
  // Attributes = GetBe32(p + 0x26);
  /*
  for (int i = 0; i < 16; i++)
    Reserved3[i] = GetBe32(p + 0x2A + i * 4);
  */
  return S_OK;
}
...
HRESULT CDatabase::LoadExtentFile(const CFork &fork, IInStream *inStream, CObjectVector<CIdExtents> *overflowExtentsArray)
{
  ...
  while (node != 0)
  {
    ...
    for (unsigned i = 0; i < desc.NumRecords; i++)
    {
      ...
      UInt32 offs = GetBe16(p + nodeOffset + nodeSize - (i + 1) * 2);
      UInt32 offsNext = GetBe16(p + nodeOffset + nodeSize - (i + 2) * 2);
      ...
      UInt32 id = GetBe32(r + 4);
      UInt32 startBlock = GetBe32(r + 8);
      r += 2 + kKeyLen;
...
HRESULT CDatabase::LoadAttrs(const CFork &fork, IInStream *inStream, IArchiveOpenCallback *progress)
{
  ...
  while (node != 0)
  {
    ...
    for (unsigned i = 0; i < desc.NumRecords; i++)
    {
      ...
      const Byte *r = p + nodeOffset + offs;
      UInt32 keyLen = GetBe16(r);

      // UInt16 pad = GetBe16(r + 2);
      UInt32 fileID = GetBe32(r + 4);
      unsigned startBlock = GetBe32(r + 8);
      if (startBlock != 0)
      {
...
HRESULT CDatabase::LoadCatalog(const CFork &fork, const CObjectVector<CIdExtents> *overflowExtentsArray, IInStream *inStream, IArchiveOpenCallback *progress)
{
  ...
  while (node != 0)
  {
    ...
    for (unsigned i = 0; i < desc.NumRecords; i++)
    {
      ...
      item.Type = type;
      // item.Flags = GetBe16(r + 2);
      // item.Valence = GetBe32(r + 4);
      item.ID = GetBe32(r + 8);
      {
      ...
      item.CTime = GetBe32(r + 0xC);
      item.MTime = GetBe32(r + 0x10);
      // item.AttrMTime = GetBe32(r + 0x14);
      item.ATime = GetBe32(r + 0x18);
      // item.BackupDate = GetBe32(r + 0x1C);
      
      /*
      item.OwnerID = GetBe32(r + 0x20);
      item.GroupID = GetBe32(r + 0x24);
      item.AdminFlags = r[0x28];
      item.OwnerFlags = r[0x29];
      item.FileMode = GetBe16(r + 0x2A);
      item.special.iNodeNum = GetBe16(r + 0x2C); // or .linkCount
      item.FileType = GetBe32(r + 0x30);
      item.FileCreator = GetBe32(r + 0x34);
      item.FinderFlags = GetBe16(r + 0x38);
      item.Point[0] = GetBe16(r + 0x3A); // v
      item.Point[1] = GetBe16(r + 0x3C); // h
      */
...
HRESULT CDatabase::Open2(IInStream *inStream, IArchiveOpenCallback *progress)
{
  ...
  if (p[0] != 'H' || (p[1] != '+' && p[1] != 'X'))
    return S_FALSE;
  h.Version = GetBe16(p + 2);
  if (h.Version < 4 || h.Version > 5)
    return S_FALSE;

  // h.Attr = GetBe32(p + 4);
  // h.LastMountedVersion = GetBe32(p + 8);
  // h.JournalInfoBlock = GetBe32(p + 0xC);

  h.CTime = GetBe32(p + 0x10);
  h.MTime = GetBe32(p + 0x14);
  // h.BackupTime = GetBe32(p + 0x18);
  // h.CheckedTime = GetBe32(p + 0x1C);

  h.NumFiles = GetBe32(p + 0x20);
  h.NumFolders = GetBe32(p + 0x24);
  ...
  UInt32 blockSize = GetBe32(p + 0x28);
  ...
  h.NumBlocks = GetBe32(p + 0x2C);
  h.NumFreeBlocks = GetBe32(p + 0x30);

  /*
  h.NextCalatlogNodeID = GetBe32(p + 0x40);
  h.WriteCount = GetBe32(p + 0x44);
  for (i = 0; i < 6; i++)
    h.FinderInfo[i] = GetBe32(p + 0x50 + i * 4);
  h.VolID = GetBe64(p + 0x68);
  */
    ...
    IArchiveExtractCallback *extractCallback)
{
  ...
  RINOK(ReadStream_FALSE(inStream, buf, kHeaderSize));
  UInt32 dataPos = GetBe32(buf);
  UInt32 mapPos = GetBe32(buf + 4);
  UInt32 dataSize = GetBe32(buf + 8);
  UInt32 mapSize = GetBe32(buf + 12);
  ...
  UInt32 dataSize2 = GetBe32(buf + 0x100);
  ...
  UInt32 types = GetBe16(buf + 24);
  UInt32 names = GetBe16(buf + 26);
  UInt32 numTypes = GetBe16(buf + 28);
  if (numTypes != 0 || types != 28 || names != kResMapSize)
    return S_FALSE;
  UInt32 resType = GetBe32(buf + 30);
  UInt32 numResources = GetBe16(buf + 34);
  UInt32 resListOffset = GetBe16(buf + 36);
  if (resType != 0x636D7066) // cmpf
    return S_FALSE;
  if (numResources != 0 || resListOffset != 10)
    return S_FALSE;

  UInt32 entryId = GetBe16(buf + 38);
  UInt32 nameOffset = GetBe16(buf + 40);
  // Byte attrib = buf[42];
  UInt32 resourceOffset = GetBe32(buf + 42) & 0xFFFFFF;
