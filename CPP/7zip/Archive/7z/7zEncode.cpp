  ...
  FOR_VECTOR (i, _options.Methods)
  {
    ...
    */
    ...
    if (cryptoSetPassword)
    {
      ...
      for (unsigned idx = 0; idx < _options.Password.Len(); idx++)
      {
        wchar_t c = _options.Password[idx];
        ((Byte *)buffer)[idx * 2] = (Byte)c;
        ((Byte *)buffer)[idx * 2 + 1] = (Byte)(c >> 8);
      }
    ...
    ICompressProgressInfo *compressProgress)
{
  ...
  */
  ...
  if (useMtProgress)
  {
    ...
    FOR_VECTOR(idx, tempBufferSpecs)
    {
      tempBufferSpecs[idx]->_mtProgresSpec = mtProgressSpec;
    }
  }
  ...
  for (size_t idx = 1; i < _bindInfo.PackStreams.Size(); idx++)
    outStreamPointers.Add(tempBuffers[idx - 1]);
...
HRESULT CEncoder::EncoderConstr()
{
  ...
  if (_bindInfo.PackStreams.Size() != 1)
  {
    ...
    for (;;)
    {
      ...
      int idx = _bindInfo.FindStream_in_PackStreams(outIndex);
      if (idx >= 0)
