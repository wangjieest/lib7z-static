    ...
    )
{
  ...
  */
  ...
  for (;;)
  {
    ...
    if (type_id > ((UInt32)1 << 30))
      isKnownType = false;
    else switch ((UInt32)type_id)
    {
      case NID::kName:
      {
        CStreamSwitch streamSwitch;
        ...
        size_t pos = 0;
        unsigned idx;
        for (idx = 0; idx < db.Files.Size(); idx++)
        {
    ...
    {
      if (addPropIdToList)
        db.ArcInfo.FileInfoPopIDs.Add(type_id);
    }
    else
    {
