...
STDMETHODIMP CHandler::UpdateItems(ISequentialOutStream *outSeqStream, UInt32 numItems, IArchiveUpdateCallback *callback)
{
  ...
  for (i = 0; i < numNewImages; i++)
  {
    ...
    {
      for (unsigned idx = 0; idx < secBufs.Size(); idx++, pos += 8)
      {
        Set64(meta + pos, secBufs[idx].Size());
      }
      for (unsigned idx = 0; idx < secBufs.Size(); idx++)
      {
        const CByteBuffer &buff = secBufs[idx];
        size_t size = buff.Size();
        if (size != 0)
        {
          memcpy(meta + pos, buff, size);
          pos += size;
        }
      }
      while ((pos & 7) != 0)
  ...
  {
    Byte buff[kStreamInfoSize];
    streams[i].WriteTo(buff);
    RINOK(WriteStream(outStream, buff, kStreamInfoSize));
