...
HRESULT CDatabase::ParseDirItem(size_t pos, int parent)
{
  ...
  for (unsigned numItems = 0;; numItems++)
  {
    ...
    {
      const size_t sub_rem = DirSize - pos;
      if (pos < DirStartOffset || pos > DirSize || sub_rem < 8)
        return S_FALSE;
      const Byte *sub_p = DirData + pos;
      const UInt64 sub_len = Get64(sub_p);
      if ((sub_len & align) != 0 || sub_rem < sub_len || sub_len < (IsOldVersion ? 0x18 : 0x28))
        return S_FALSE;
     
      DirProcessed += (size_t)sub_len;
      if (DirProcessed > DirSize)
        return S_FALSE;
      ...
      UInt32 sub_fileNameLen = Get16(sub_p + extraOffset);
      if ((sub_fileNameLen & 1) != 0)
        return S_FALSE;
      /* Probably different versions of ImageX can use different number of
         additional ZEROs. So we don't use exact check. */
      UInt32 sub_fileNameLen2 = (sub_fileNameLen == 0 ? sub_fileNameLen : sub_fileNameLen + 2);
      if (((extraOffset + 2 + sub_fileNameLen2 + align) & ~align) > sub_len)
        return S_FALSE;
      
      {
        const Byte *p2 = sub_p + extraOffset + 2;
        if (*(const UInt16 *)(p2 + sub_fileNameLen) != 0)
          return S_FALSE;
        for (UInt32 j = 0; j < sub_fileNameLen; j += 2)
          if (*(const UInt16 *)(p2 + j) == 0)
            return S_FALSE;
      ...
      if (sub_fileNameLen == 0 &&
          ((attrib & FILE_ATTRIBUTE_REPARSE_POINT) || !item.IsDir)
          && (IsOldVersion || IsEmptySha(prevMeta + 0x40)))
      {
        if (IsOldVersion)
          memcpy(prevMeta + 0x10, sub_p + 8, 4); // It's 32-bit Id
        else if (!IsEmptySha(sub_p + 0x10))
        {
          // if (IsEmptySha(prevMeta + 0x40))
            memcpy(prevMeta + 0x40, sub_p + 0x10, kHashSize);
