  ...
  switch (propID)
  {
    ...
    case kpidCTime:
      if (_xmls.Size() == 1)
      {
        ...
        {
          const CImageInfo &sub_image = xml.Images[i];
          if (sub_image.CTimeDefined)
            if (index < 0 || ::CompareFileTime(&sub_image.CTime, &xml.Images[index].CTime) < 0)
              index = i;
        }
        if (index >= 0)
    ...
    case kpidMTime:
      if (_xmls.Size() == 1)
      {
        ...
        {
          const CImageInfo &sub_image = xml.Images[i];
          if (sub_image.MTimeDefined)
            if (index < 0 || ::CompareFileTime(&sub_image.MTime, &xml.Images[index].MTime) > 0)
              index = i;
        }
        if (index >= 0)
