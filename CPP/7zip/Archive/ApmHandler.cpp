...
struct CItem
{
  ...
  */
  ...
  {
    numBlocksInMap = GetBe32(p + 4);
    StartBlock = GetBe32(p + 8);
    NumBlocks = GetBe32(p + 0xC);
    memcpy(Name, p + 0x10, 32);
    memcpy(Type, p + 0x30, 32);
    if (p[0] != 0x50 || p[1] != 0x4D || p[2] != 0 || p[3] != 0)
      return false;
    /*
    DataStartBlock = GetBe32(p + 0x50);
    NumDataBlocks = GetBe32(p + 0x54);
    Status = GetBe32(p + 0x58);
    BootStartBlock = GetBe32(p + 0x5C);
    BootSize = GetBe32(p + 0x60);
    BootAddr = GetBe32(p + 0x64);
    if (GetBe32(p + 0x68) != 0)
      return false;
    BootEntry = GetBe32(p + 0x6C);
    if (GetBe32(p + 0x70) != 0)
      return false;
    BootChecksum = GetBe32(p + 0x74);
...
HRESULT CHandler::ReadTables(IInStream *stream)
{
  Byte buf[kSectorSize];
  {
    ...
    if (buf[0] != kSig0 || buf[1] != kSig1)
      return S_FALSE;
    UInt32 blockSize = GetBe16(buf + 2);
    unsigned i;
    for (i = 9; ((UInt32)1 << i) != blockSize; i++)
      if (i >= 12)
        return S_FALSE;
    _blockSizeLog = i;
    _numBlocks = GetBe32(buf + 4);
    for (i = 8; i < 16; i++)
      if (buf[i] != 0)
        return S_FALSE;
  }
