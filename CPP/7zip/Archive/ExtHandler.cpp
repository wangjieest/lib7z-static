...
#define NExtGet16(p) GetUi16(p)
#define NExtGet32(p) GetUi32(p)
#define NExtGet64(p) GetUi64(p)

#define NExtLE_16(offs, dest) dest = NExtGet16(p + (offs));
#define NExtLE_32(offs, dest) dest = NExtGet32(p + (offs));
#define NExtLE_64(offs, dest) dest = NExtGet64(p + (offs));

#define NExtHI_16(offs, dest) dest |= (((UInt32)NExtGet16(p + (offs))) << 16);
#define NExtHI_32(offs, dest) dest |= (((UInt64)NExtGet32(p + (offs))) << 32);
...
bool CHeader::Parse(const Byte *p)
{
  ...
  NExtLE_32 (0x18, BlockBits);
  NExtLE_32 (0x1C, ClusterBits);
  ...
  NExtLE_32 (0x00, NumInodes);
  NExtLE_32 (0x04, NumBlocks);
  // NExtLE_32 (0x08, NumBlocksSuper);
  NExtLE_32 (0x0C, NumFreeBlocks);
  NExtLE_32 (0x10, NumFreeInodes);

  if (NumInodes < 2 || NumInodes <= NumFreeInodes)
    return false;

  UInt32 FirstDataBlock;
  NExtLE_32 (0x14, FirstDataBlock);
  if (FirstDataBlock != (unsigned)(BlockBits == 10 ? 1 : 0))
    return false;
    
  NExtLE_32 (0x20, BlocksPerGroup);
  NExtLE_32 (0x24, ClustersPerGroup);
  ...
  NExtLE_32 (0x28, InodesPerGroup);

  if (InodesPerGroup < 1 || InodesPerGroup > NumInodes)
    return false;
  
  NExtLE_32 (0x2C, MountTime);
  NExtLE_32 (0x30, WriteTime);
  
  // NExtLE_16 (0x34, NumMounts);
  // NExtLE_16 (0x36, NumMountsMax);
  
  // NExtLE_16 (0x3A, State);
  // NExtLE_16 (0x3C, Errors);
  // NExtLE_16 (0x3E, MinorRevLevel);
  
  NExtLE_32 (0x40, LastCheckTime);
  // NExtLE_32 (0x44, CheckInterval);
  NExtLE_32 (0x48, CreatorOs);
  NExtLE_32 (0x4C, RevLevel);
  
  // NExtLE_16 (0x50, DefResUid);
  // NExtLE_16 (0x52, DefResGid);
  ...
  if (!IsOldRev())
  {
    NExtLE_32 (0x54, FirstInode);
    NExtLE_16 (0x58, InodeSize);
  ...
  NExtLE_16 (0x5A, BlockGroupNr);
  NExtLE_32 (0x5C, FeatureCompat);
  NExtLE_32 (0x60, FeatureIncompat);
  NExtLE_32 (0x64, FeatureRoCompat);
  
  memcpy(Uuid, p + 0x68, sizeof(Uuid));
  memcpy(VolName, p + 0x78, sizeof(VolName));
  memcpy(LastMount, p + 0x88, sizeof(LastMount));
  
  // NExtLE_32 (0xC8, BitmapAlgo);
  
  NExtLE_32 (0xE0, JournalInode);
  
  NExtLE_16 (0xFE, GdSize);
  
  NExtLE_32 (0x108, CTime);

  if (Is64Bit())
  {
    NExtHI_32(150, NumBlocks);
    // NExtHI_32(154, NumBlocksSuper);
    NExtHI_32(0x158, NumFreeBlocks);
  }
  ...
  NExtLE_16(0x15C, MinExtraISize);
  // NExtLE_16(0x15E, WantExtraISize);
  // NExtLE_32(0x160, Flags);
  // NExtLE_16(0x164, RaidStride);
  // NExtLE_16(0x166, MmpInterval);
  // NExtLE_64(0x168, MmpBlock);
  
  // LogGroupsPerFlex = p[0x174];
  // ChecksumType = p[0x175];

  NExtLE_64 (0x178, WrittenKB);

  // NExtLE_32(0x194, ErrorCount);
  // NExtLE_32(0x198, ErrorTime);
  // NExtLE_32(0x19C, ErrorINode);
  // NExtLE_32(0x1A0, ErrorBlock);
...
{
  NExtLE_32 (0x00, BlockBitmap);
  NExtLE_32 (0x04, InodeBitmap);
  NExtLE_32 (0x08, InodeTable);
  NExtLE_16 (0x0C, NumFreeBlocks);
  NExtLE_16 (0x0E, NumFreeInodes);
  NExtLE_16 (0x10, DirCount);
  NExtLE_16 (0x12, Flags);
  NExtLE_32 (0x14, ExcludeBitmap);
  NExtLE_16 (0x18, BlockBitmap_Checksum);
  NExtLE_16 (0x1A, InodeBitmap_Checksum);
  NExtLE_16 (0x1C, UnusedCount);
  NExtLE_16 (0x1E, Checksum);
  
  if (size >= 64)
  {
    p += 0x20;
    NExtHI_32 (0x00, BlockBitmap);
    NExtHI_32 (0x04, InodeBitmap);
    NExtHI_32 (0x08, InodeTable);
    NExtHI_16 (0x0C, NumFreeBlocks);
    NExtHI_16 (0x0E, NumFreeInodes);
    NExtHI_16 (0x10, DirCount);
    NExtHI_16 (0x12, UnusedCount); // instead of Flags
    NExtHI_32 (0x14, ExcludeBitmap);
    NExtHI_16 (0x18, BlockBitmap_Checksum);
    NExtHI_16 (0x1A, InodeBitmap_Checksum);
    // NExtHI_16 (0x1C, Reserved);
  }
}
...
struct CExtentTreeHeader
{
  ...
  bool Parse(const Byte *p)
  {
    NExtLE_16 (0x02, NumEntries);
    NExtLE_16 (0x04, MaxEntries);
    NExtLE_16 (0x06, Depth);
    // NExtLE_32 (0x08, Generation);
    return NExtGet16(p) == 0xF30A; // magic
  }
};
...
struct CExtentIndexNode
{
  ...
  {
    NExtLE_32 (0x00, VirtBlock);
    NExtLE_32 (0x04, PhyLeaf);
    PhyLeaf |= (((UInt64)NExtGet16(p + 8)) << 32);
...
bool CNode::Parse(const Byte *p, const CHeader &_h)
{
  ...
  NExtLE_16 (0x00, Mode);
  NExtLE_16 (0x02, Uid);
  NExtLE_32 (0x04, FileSize);
  NExtLE_32 (0x08, ATime.Val);
  // NExtLE_32 (0x0C, InodeChangeTime.Val);
  NExtLE_32 (0x10, MTime.Val);
  // NExtLE_32 (0x14, DTime.Val);
  NExtLE_16 (0x18, Gid);
  NExtLE_16 (0x1A, NumLinks);

  NExtLE_32 (0x1C, NumBlocks);
  NExtLE_32 (0x20, Flags);
  // NExtLE_32 (0x24, Union osd1);
  
  memcpy(Block, p + 0x28, kNodeBlockFieldSize);
  
  // NExtLE_32 (0x64, Generation);  // File version (for NFS)
  // NExtLE_32 (0x68, ACL);
  
  {
    UInt32 highSize;
    NExtLE_32 (0x6C, highSize); // In ext2/3 this field was named i_dir_acl
  ...
  // NExtLE_32 (0x70, fragmentAddress);
  ...
  // osd2
  {
    ...
    NExtLE_16 (0x74, numBlocksHigh);
    NumBlocks |= (UInt64)numBlocksHigh << 32;
    
    NExtHI_16 (0x74 + 4, Uid);
    NExtHI_16 (0x74 + 6, Gid);
    /*
    UInt32 checksum;
    NExtLE_16 (0x74 + 8, checksum);
    checksum = checksum;
    */
  }
  ...
  {
    UInt16 extra_isize;
    NExtLE_16 (0x80, extra_isize);
    if (128 + extra_isize > _h.InodeSize)
      return false;
    if (extra_isize >= 0x1C)
    {
      // UInt16 checksumUpper;
      // NExtLE_16 (0x82, checksumUpper);
      // NExtLE_32 (0x84, InodeChangeTime.Extra);
      NExtLE_32 (0x88, MTime.Extra);
      NExtLE_32 (0x8C, ATime.Extra);
      NExtLE_32 (0x90, CTime.Val);
      NExtLE_32 (0x94, CTime.Extra);
      // NExtLE_32 (0x98, VersionHi);
    }
  }
...
HRESULT CHandler::ParseDir(const Byte *p, size_t size, unsigned iNodeDir)
{
  ...
  for (;;)
  {
    ...
    if (size < 8)
      return S_FALSE;
    UInt32 iNode;
    NExtLE_32 (0x00, iNode);
    unsigned recLen;
    NExtLE_16 (0x04, recLen);
