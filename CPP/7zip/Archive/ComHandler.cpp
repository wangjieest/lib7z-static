...
static const Byte kSignature[] = { 0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1 };
...
void CItem::Parse(const Byte *p, bool mode64bit)
{
  ...
  // NameSize = GetUi16(p + 64);
  Type = p[66];
  LeftDid = GetUi32(p + 68);
  RightDid = GetUi32(p + 72);
  SonDid = GetUi32(p + 76);
  // Flags = GetUi32(p + 96);
...
HRESULT CDatabase::Open(IInStream *inStream)
{
  ...
  if (memcmp(p, kSignature, ARRAY_SIZE(kSignature)) != 0)
    return S_FALSE;
  if (GetUi16(p + 0x1A) > 4) // majorVer
    return S_FALSE;
  if (GetUi16(p + 0x1C) != 0xFFFE) // Little-endian
    return S_FALSE;
  unsigned sectorSizeBits = GetUi16(p + 0x1E);
  bool mode64bit = (sectorSizeBits >= 12);
  unsigned miniSectorSizeBits = GetUi16(p + 0x20);
  ...
  if (sectorSizeBits > 24 ||
    ...
    return S_FALSE;
  UInt32 numSectorsForFAT = GetUi32(p + 0x2C); // SAT
  LongStreamMinSize = GetUi32(p + 0x38);
  ...
  {
    UInt32 numSectorsForBat = GetUi32(p + 0x48); // master sector allocation table
  ...
  UInt32 numMatItems;
  {
    ...
    UInt32 i;
    UInt32 sid = GetUi32(p + 0x3C); // short-sector table SID
...

  {
    ...
    for (UInt32 i = 0; i < numFatItems; i++)
      used[i] = 0;
    UInt32 sid = GetUi32(p + 0x30); // directory stream SID
    for (;;)
    {
      if (sid >= numFatItems)
        return S_FALSE;
