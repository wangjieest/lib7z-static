...
static const Byte k_Signature[k_SignatureSize] = { 'E', 'F', 'I', ' ', 'P', 'A', 'R', 'T', 0, 0, 1, 0 };
...
struct CPartition
{
  ...
  void Parse(const Byte *p)
  {
    memcpy(Type, p, 16);
    memcpy(Id, p + 16, 16);
    FirstLba = GetUi64(p + 32);
    LastLba = GetUi64(p + 40);
    Flags = GetUi64(p + 48);
...
HRESULT CHandler::Open2(IInStream *stream)
{
  ...
  if (memcmp(buf, k_Signature, k_SignatureSize) != 0)
    return S_FALSE;
  {
    // if (GetUi32(buf + 8) != 0x10000) return S_FALSE; // revision
    UInt32 headerSize = GetUi32(buf + 12); // = 0x5C usually
    if (headerSize > kSectorSize)
      return S_FALSE;
    UInt32 crc = GetUi32(buf + 0x10);
    SetUi32(_buffer + kSectorSize + 0x10, 0);
    if (CrcCalc(_buffer + kSectorSize, headerSize) != crc)
      return S_FALSE;
  }
  // UInt32 reserved = GetUi32(buf + 0x14);
  UInt64 curLba = GetUi64(buf + 0x18);
  if (curLba != 1)
    return S_FALSE;
  UInt64 backupLba = GetUi64(buf + 0x20);
  // UInt64 firstUsableLba = GetUi64(buf + 0x28);
  // UInt64 lastUsableLba = GetUi64(buf + 0x30);
  memcpy(Guid, buf + 0x38, 16);
  UInt64 tableLba = GetUi64(buf + 0x48);
  if (tableLba < 2)
    return S_FALSE;
  UInt32 numEntries = GetUi32(buf + 0x50);
  UInt32 entrySize = GetUi32(buf + 0x54); // = 128 usually
  UInt32 entriesCrc = GetUi32(buf + 0x58);
...
STDMETHODIMP CHandler::GetProperty(UInt32 index, PROPID propID, PROPVARIANT *value)
{
  ...
  switch (propID)
  {
    case kpidPath:
    {
      ...
      {
        wchar_t c = (wchar_t)GetUi16(item.Name + i * 2);
        if (c == 0)
          break;
        s += c;
      }
