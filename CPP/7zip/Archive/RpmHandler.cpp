...
HRESULT CHandler::ReadHeader(ISequentialInStream *stream, bool isMainHeader)
{
  ...
  UInt32 dataLen;
  {
    ...
    if (GetBe32(buf) != 0x8EADE801) // buf[3] = 0x01 - is version
      return S_FALSE;
    // reserved = GetBe32(buf + 4);
    numEntries = GetBe32(buf + 8);
    dataLen = GetBe32(buf + 12);
    if (numEntries >= 1 << 24)
      return S_FALSE;
  }
    ...
    if (!isMainHeader)
    {
          ...
          entry.Type == k_EntryType_INT32)
      {
        if (rem < 4 || entry.Count != 1)
          return S_FALSE;
        _headerPlusPayload_Size = GetBe32(p);
      ...
      else if (entry.Type == k_EntryType_INT32)
      {
        ...
        for (UInt32 t = 0; t < entry.Count; t++)
        {
          if (t != 0)
            _metadata.Add_Space();
          char temp[16];
          ConvertUInt32ToString(GetBe32(p + t * 4), temp);
          _metadata += temp;
        }
        #endif
      }
      ...
      else if (entry.Type == k_EntryType_INT16)
      {
        ...
        for (UInt32 t = 0; t < entry.Count; t++)
        {
          if (t != 0)
            _metadata.Add_Space();
          char temp[16];
          ConvertUInt32ToString(GetBe16(p + t * 2), temp);
          _metadata += temp;
        }
      }
