...
bool CHeader::Parse(const Byte *p)
{
  ...
  NumReservedSectors = GetUi16(p + 14);
  ...
  UInt16 numRootDirEntries = GetUi16(p + 17);
  ...
  NumSectors = GetUi16(p + 19);
  if (NumSectors == 0)
    NumSectors = GetUi32(p + 32);
  else if (IsFat32())
    return false;

  MediaType = p[21];
  NumFatSectors = GetUi16(p + 22);
  SectorsPerTrack = GetUi16(p + 24);
  NumHeads = GetUi16(p + 26);
  NumHiddenSectors = GetUi32(p + 28);
  ...
  if (IsFat32())
  {
    ...
    if (GetUi16(p + 6) != 0)
      return false;
    RootCluster = GetUi32(p + 8);
    FsInfoSector = GetUi16(p + 12);
    for (int i = 16; i < 28; i++)
      if (p[i] != 0)
        return false;
    p += 28;
...
HRESULT CDatabase::ReadDir(Int32 parent, UInt32 cluster, unsigned level)
{
  ...
  for (UInt32 pos = blockSize;; pos += 32)
  {
    ...
    else
    {
      ...
      item.Attrib = attrib;
      item.Flags = p[12];
      item.Size = GetUi32(p + 28);
      item.Cluster = GetUi16(p + 26);
      if (Header.NumFatBits > 16)
        item.Cluster |= ((UInt32)GetUi16(p + 20) << 16);
      else
      {
        // OS/2 and WinNT probably can store EA (extended atributes) in that field.
      }

      item.CTime = GetUi32(p + 14);
      item.CTime2 = p[13];
      item.ADate = GetUi16(p + 18);
      item.MTime = GetUi32(p + 22);
      item.Parent = parent;
...
HRESULT CDatabase::Open()
{
    ...
    */
    ...
    if (Header.IsFat32())
    {
      ...
      if (buf[0x1FE] != 0x55 || buf[0x1FF] != 0xAA)
        return S_FALSE;
      if (GetUi32(buf) == 0x41615252 && GetUi32(buf + 484) == 0x61417272)
      {
        NumFreeClusters = GetUi32(buf + 488);
    ...
    for (UInt32 i = 0; i < Header.FatSize;)
    {
      ...
      if (numFreeClustersDefined)
        for (UInt32 j = 0; j < size; j++)
          dest[j] = GetUi32(src + j) & 0x0FFFFFFF;
      else
      {
        UInt32 numFreeClusters = 0;
        for (UInt32 j = 0; j < size; j++)
        {
          UInt32 v = GetUi32(src + j) & 0x0FFFFFFF;
  ...
  else
  {
    ...
    if (Header.NumFatBits == 16)
      ...
      for (UInt32 j = 0; j < fatSize; j++)
        fat[j] = (GetUi16(p + j * 3 / 2) >> ((j & 1) << 2)) & 0xFFF;
