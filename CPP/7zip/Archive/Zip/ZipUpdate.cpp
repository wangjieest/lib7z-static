    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  for (unsigned i = 0; i < updateItems.Size(); i++)
  {
...

  {
    ...
    for (unsigned idx = 0; idx < updateItems.Size(); idx++)
      refs.Refs.Add(CMemBlocks2());

    for (UInt32 idx = 0; idx < numThreads; idx++)
      threads.Threads.Add(CThreadInfo(options2));

    for (UInt32 idx = 0; idx < numThreads; idx++)
    {
      CThreadInfo &threadInfo = threads.Threads[idx];
      ...
      threadInfo.ProgressSpec->Init(&mtCompressProgressMixer, (int)idx);
  ...
  while (itemIndex < updateItems.Size())
  {
    if (threadIndices.Size() < numThreads && mtItemIndex < updateItems.Size())
    {
      ...
      for (UInt32 idx = 0; idx < numThreads; idx++)
      {
        CThreadInfo &sub_threadInfo = threads.Threads[idx];
        if (sub_threadInfo.IsFree)
        {
          sub_threadInfo.IsFree = false;
          sub_threadInfo.InStream = fileInStream;
          ...
          sub_threadInfo.OutStreamSpec->Init();
          sub_threadInfo.ProgressSpec->Reinit();
          sub_threadInfo.CompressEvent.Set();
          sub_threadInfo.UpdateIndex = mtItemIndex - 1;

          compressingCompletedEvents.Add(sub_threadInfo.CompressionCompletedEvent);
          threadIndices.Add(idx);
          break;
        }
      }
    ...
    if (ui.NewData)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            CMemBlocks2 &sub_memRef = refs.Refs[threadInfo.UpdateIndex];
            threadInfo.OutStreamSpec->DetachData(sub_memRef);
            sub_memRef.CompressingResult = threadInfo.CompressingResult;
            sub_memRef.Defined = true;
            continue;
          }
        }
      }
