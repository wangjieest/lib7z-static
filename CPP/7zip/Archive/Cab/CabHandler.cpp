    ...
    Int32 testModeSpec, IArchiveExtractCallback *extractCallback)
{
  ...
  for (i = 0;;)
  {
    ...
    for (; i < numItems; i++)
    {
      ...
      const CMvItem &sub_mvItem = m_Database.Items[indexNext];
      const CItem &sub_item = m_Database.Volumes[sub_mvItem.VolumeIndex].Items[sub_mvItem.ItemIndex];
      if (sub_item.IsDir())
        continue;
      int newFolderIndex = m_Database.GetFolderIndex(&sub_mvItem);
      ...
      extractStatuses.Add(true);
      startIndex++;
      curUnpack = sub_item.GetEndOffset();
    }
...

    {
      ...
      for (UInt32 bl = 0; cabFolderOutStream->NeedMoreWrite();)
      {
        ...
        const CDatabaseEx &sub_db = m_Database.Volumes[volIndex];
        const CFolder &sub_folder = sub_db.Folders[locFolderIndex];
        
        if (bl == 0)
        {
          cabBlockInStreamSpec->ReservedSize = sub_db.ArcInfo.GetDataBlockReserveSize();
          RINOK(sub_db.Stream->Seek(sub_db.StartPosition + sub_folder.DataStart, STREAM_SEEK_SET, NULL));
        }
        
        if (bl == sub_folder.NumDataBlocks)
        {
          /*
        ...
        res = cabBlockInStreamSpec->PreRead(sub_db.Stream, packSize, unpackSize);
        if (res == S_FALSE)
          break;
        RINOK(res);
