...
#define NtfsPRF(x) x
#define NtfsPRF_UTF16(x) NtfsPRF(printf("%S", x))
#else
#define NtfsPRF(x)
#define NtfsPRF_UTF16(x)
#endif
...
#define NtfsG16(p, dest) dest = GetUi16(p);
#define NtfsG32(p, dest) dest = GetUi32(p);
#define NtfsG64(p, dest) dest = GetUi64(p);
...
bool CHeader::Parse(const Byte *p)
{
  ...
  switch (p[0])
  {
    case 0xE9: /* codeOffset = 3 + (Int16)GetUi16(p + 1); */ break;
  ...
  if (p[21] != 0xF8) // MediaType = Fixed_Disk
    return false;
  if (GetUi16(p + 22) != 0) // NumFatSectors
    return false;
  NtfsG16(p + 24, SectorsPerTrack); // 63 usually
  NtfsG16(p + 26, NumHeads); // 255
  NtfsG32(p + 28, NumHiddenSectors); // 63 (XP) / 2048 (Vista and win7) / (0 on media that are not partitioned ?)
  if (GetUi32(p + 32) != 0) // NumSectors32
    return false;
  ...
  */
  ...
  NumSectors = GetUi64(p + 0x28);
  if (NumSectors >= ((UInt64)1 << (62 - SectorSizeLog)))
    return false;

  NumClusters = NumSectors >> sectorsPerClusterLog;

  NtfsG64(p + 0x30, MftCluster);
  // NtfsG64(p + 0x38, Mft2Cluster);
  NtfsG64(p + 0x48, SerialNumber);
  UInt32 numClustersInMftRec;
  UInt32 numClustersInIndexBlock;
  NtfsG32(p + 0x40, numClustersInMftRec); // -10 means 2 ^10 = 1024 bytes.
  NtfsG32(p + 0x44, numClustersInIndexBlock);
...
bool CFileNameAttr::Parse(const Byte *p, unsigned size)
{
  if (size < 0x42)
    return false;
  NtfsG64(p + 0x00, ParentDirRef.Val);
  // NtfsG64(p + 0x08, CTime);
  // NtfsG64(p + 0x10, MTime);
  // NtfsG64(p + 0x18, ThisRecMTime);
  // NtfsG64(p + 0x20, ATime);
  // NtfsG64(p + 0x28, AllocatedSize);
  // NtfsG64(p + 0x30, DataSize);
  NtfsG32(p + 0x38, Attrib);
  // NtfsG16(p + 0x3C, PackedEaSize);
  NameType = p[0x41];
  unsigned len = p[0x40];
  if (0x42 + len > size)
    return false;
...
bool CSiAttr::Parse(const Byte *p, unsigned size)
{
  if (size < 0x24)
    return false;
  NtfsG64(p + 0x00, CTime);
  NtfsG64(p + 0x08, MTime);
  // NtfsG64(p + 0x10, ThisRecMTime);
  NtfsG64(p + 0x18, ATime);
  NtfsG32(p + 0x20, Attrib);
  SecurityId = 0;
  if (size >= 0x38)
    NtfsG32(p + 0x34, SecurityId);
  return true;
}
...
UInt32 CAttr::Parse(const Byte *p, unsigned size)
{
  ...
  if (size < 0x18)
    return 0;
  NtfsPRF(printf(" T=%2X", Type));
  
  UInt32 len = GetUi32(p + 0x04);
  NtfsPRF(printf(" L=%3d", len));
  if (len > size)
    return 0;
  if ((len & 7) != 0)
    return 0;
  NonResident = p[0x08];
  {
    ...
    UInt32 nameOffset = GetUi16(p + 0x0A);
    if (nameLen != 0)
    {
  ...
  // NtfsG16(p + 0x0C, Flags);
  // NtfsG16(p + 0x0E, Instance);
  // NtfsPRF(printf(" F=%4X", Flags));
  // NtfsPRF(printf(" Inst=%d", Instance));
  ...
  if (NonResident)
  {
    if (len < 0x40)
      return 0;
    NtfsPRF(printf(" NR"));
    NtfsG64(p + 0x10, LowVcn);
    NtfsG64(p + 0x18, HighVcn);
    NtfsG64(p + 0x28, AllocatedSize);
    NtfsG64(p + 0x30, Size);
    NtfsG64(p + 0x38, InitializedSize);
    NtfsG16(p + 0x20, offs);
    ...
    if (CompressionUnit != 0)
    {
      if (len < 0x48)
        return 0;
      NtfsG64(p + 0x40, PackSize);
      NtfsPRF(printf(" PS=%I64x", PackSize));
    }

    // NtfsPRF(printf("\n"));
    NtfsPRF(printf(" ASize=%4I64d", AllocatedSize));
    NtfsPRF(printf(" Size=%I64d", Size));
    NtfsPRF(printf(" IS=%I64d", InitializedSize));
    NtfsPRF(printf(" Low=%I64d", LowVcn));
    NtfsPRF(printf(" High=%I64d", HighVcn));
    NtfsPRF(printf(" CU=%d", (unsigned)CompressionUnit));
    dataSize = len - offs;
  }
  else
  {
    if (len < 0x18)
      return 0;
    NtfsPRF(printf(" RES"));
    dataSize = GetUi32(p + 0x10);
    NtfsPRF(printf(" dataSize=%3d", dataSize));
    offs = GetUi16(p + 0x14);
    // NtfsG16(p + 0x16, ResidentFlags);
    // NtfsPRF(printf(" ResFlags=%4X", ResidentFlags));
  }
  ...
  {
    NtfsPRF(printf(" %02X", (unsigned)Data[i]));
  }
  #endif
  return len;
}
    ...
    unsigned attrIndex, unsigned attrIndexLim, UInt64 numPhysClusters, CRecordVector<CExtent> &Extents)
{
  ...
  FOR_VECTOR (k, Extents)
  {
    ...
    NtfsPRF2(printf("  Pos = %4I64X", e.Phy));
  }
    ...
    CObjectVector<CAttr> *attrs)
{
  ...
  
  {
    UInt32 usaOffset;
    UInt32 numUsaItems;
    NtfsG16(p + 0x04, usaOffset);
    NtfsG16(p + 0x06, numUsaItems);
    ...
    if (usaOffset >= 0x30) // NTFS 3.1+
      if (GetUi32(p + 0x2C) != recNumber)
        return false;
    
    UInt16 usn = GetUi16(p + usaOffset);
    // NtfsPRF(printf("\nusn = %d", usn));
    for (UInt32 i = 1; i < numUsaItems; i++)
    {
      ...
      if (GetUi16(pp) != usn)
        return false;
      SetUi16(pp, GetUi16(p + usaOffset + i * 2));
    }
  }

  // NtfsG64(p + 0x08, Lsn);
  NtfsG16(p + 0x10, SeqNumber);
  // NtfsG16(p + 0x12, LinkCount);
  // NtfsPRF(printf(" L=%d", LinkCount));
  UInt32 attrOffs = GetUi16(p + 0x14);
  NtfsG16(p + 0x16, Flags);
  NtfsPRF(printf(" F=%4X", Flags));

  UInt32 bytesInUse = GetUi32(p + 0x18);
  UInt32 bytesAlloc = GetUi32(p + 0x1C);
  NtfsG64(p + 0x20, BaseMftRef.Val);
  if (BaseMftRef.Val != 0)
  {
    NtfsPRF(printf("  BaseRef=%d", (int)BaseMftRef.Val));
    // return false; // Check it;
  }
  // NtfsG16(p + 0x28, NextAttrInstance);
    ...
    switch (attr.Type)
    {
      case ATTR_TYPE_FILE_NAME:
      {
        ...
        if (!attr.ParseFileName(fna))
          return false;
        FileNames.Add(fna);
        NtfsPRF(printf("  flags = %4x\n  ", (int)fna.NameType));
        NtfsPRF_UTF16(fna.Name);
        break;
      }
...
bool CDatabase::FindSecurityDescritor(UInt32 item, UInt64 &offset, UInt32 &size) const
{
  ...
  while (left != right)
  {
    ...
    UInt32 midValue = GetUi32(((const Byte *)SecurData) + offs + 4);
    if (item == midValue)
    {
      offset = GetUi64((const Byte *)SecurData + offs + 8) + 20;
      size = GetUi32((const Byte *)SecurData + offs + 16) - 20;
      return true;
    }
    if (item < midValue)
      right = mid;
...
{
  UInt32 id1 = GetUi32(((const Byte *)data) + *p1 + 4);
  UInt32 id2 = GetUi32(((const Byte *)data) + *p2 + 4);
...
HRESULT CDatabase::ParseSecuritySDS_2()
{
  ...
  {
    UInt32 id = GetUi32(p + pos + 4);
    UInt64 offs = GetUi64(p + pos + 8);
    UInt32 entrySize = GetUi32(p + pos + 16);
...
HRESULT CDatabase::Open()
{
  ...
  */
  ...
  CMyComPtr<IInStream> mftStream;
  {
    ...
    {
      UInt32 allocSize = GetUi32(ByteBuf + 0x1C);
  ...
  for (i = 0; i < SecurityAttrs.Size(); i++)
  {
    ...
    if (attr.Name == L"$SII")
    {
      if (attr.Type == ATTR_TYPE_INDEX_ROOT)
      {
        ...
        // Index Root
        UInt32 attrType = GetUi32(data);
        UInt32 collationRule = GetUi32(data + 4);
        UInt32 indexAllocationEtrySizeSize = GetUi32(data + 8);
        UInt32 clustersPerIndexRecord = GetUi32(data + 0xC);
        data += 0x10;

        // Index Header
        UInt32 firstEntryOffset = GetUi32(data);
        UInt32 totalSize = GetUi32(data + 4);
        UInt32 allocSize = GetUi32(data + 8);
        UInt32 flags = GetUi32(data + 0xC);

        int num = 0;
        for (int j = 0 ; j < num; j++)
        {
          if (GetUi32(data) != 0x1414 || // offset and size
              GetUi32(data + 4) != 0 ||
              GetUi32(data + 8) != 0x428) // KeySize / EntrySize
            break;
          UInt32 flags = GetUi32(data + 12);
          UInt32 id = GetUi32(data + 0x10);
          if (id = GetUi32(data + 0x18))
            break;
          UInt32 descriptorOffset = GetUi64(data + 0x1C);
          UInt32 descriptorSize = GetUi64(data + 0x24);
          data += 0x28;
        }
        // break;
      }
