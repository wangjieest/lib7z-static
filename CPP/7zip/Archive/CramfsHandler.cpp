...
static const Byte kSignature[] = { 'C','o','m','p','r','e','s','s','e','d',' ','R','O','M','F','S' };
...
struct CNode
{
  ...
  void Parse(const Byte *p)
  {
    Mode = GetUi16(p);
    Uid = GetUi16(p + 2);
    Size = NCramfsGet32(p + 4) & 0xFFFFFF;
    Gid = p[7];
    NameLen = p[8] & 0x3F;
    Offset = NCramfsGet32(p + 8) >> 6;
  }
};
*/

#define NCramfsGet32(p) (be ? GetBe32(p) : GetUi32(p))
...
struct CHeader
{
  ...
  bool Parse(const Byte *p)
  {
    ...
    Size = NCramfsGet32(p + 4);
    Flags = NCramfsGet32(p + 8);
    // Future = NCramfsGet32(p + 0xC);
    Crc = NCramfsGet32(p + 0x20);
    // Edition = NCramfsGet32(p + 0x24);
    NumBlocks = NCramfsGet32(p + 0x28);
    NumFiles = NCramfsGet32(p + 0x2C);
...
HRESULT CHandler::Open2(IInStream *inStream)
{
  ...
  if (!_h.IsVer2())
  {
    FOR_VECTOR (i, _items)
    {
      ...
      if (start > _size)
        continue;
      UInt32 end = NCramfsGet32(_data + start - 4);
      if (end >= start)
        UpdatePhySize(end);
    }
...
HRESULT CHandler::ReadBlock(UInt64 blockIndex, Byte *dest, size_t blockSize)
{
  ...
  UInt32 start = (blockIndex == 0 ? _curBlocksOffset + _curNumBlocks * 4: NCramfsGet32(p - 4));
  UInt32 end = NCramfsGet32(p);
...
STDMETHODIMP CHandler::GetStream(UInt32 index, ISequentialInStream **stream)
{
  ...
  {
    UInt32 next = NCramfsGet32(_data + offset + i * 4);
